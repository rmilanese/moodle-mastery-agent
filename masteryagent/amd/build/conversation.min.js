...
define("mod_masteryagent/conversation", ["require", "exports", "core/ajax", "core_filters/events"], function (require, exports, ajax_1, events_1) {
    "use strict";
    ...
    const init = selector => {
        ...
        root.addEventListener('submit', async (event) => {
            ...
            const previousMessages = new Set(Array.from(content.querySelectorAll('[data-message-id]'), node => node.dataset.messageId));
            error.hidden = true;
            error.textContent = '';
            setBusy(true);
            try {
                // core/ajax supplies the Moodle session key and standard authenticated endpoint.
                const response = await (0, ajax_1.call)([{
                ...
                const historyState = new Map(Array.from(content.querySelectorAll('[data-region="lesson-history"]'), node => [node.dataset.sectionKey, node.open]));
                const browsing = document.activeElement;
                const preserveFocus = content.contains(browsing) && browsing.closest('[data-region="lesson-history"], [data-region="conversation-navigation"], [data-region="current-lesson"]');
                ...
                (0, events_1.notifyFilterContentUpdated)([content]);
            }
            catch (exception) {
...
//# sourceMappingURL=conversation.min.js.map